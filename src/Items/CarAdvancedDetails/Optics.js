...
function Optics(){
    ...
    async function loadData(){
        const responseRegion = await fetch("http://localhost:8081/api/carOptics/getAllCarOptics",{
            method:"GET",
            headers:{
    ...
    async function createBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carOptics/addCarOptics",{
            method:"POST",
            mode:"cors",
            cache:"no-cache",
    ...
    async function editBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carOptics/saveCarOptics",{
            method:"PUT",
            mode:"cors",
            cache:"no-cache",
            credentials:"same-origin",
    ...
    async function deleteBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carOptics/deleteCarOptics",{
            method:"DELETE",
            mode:"cors",
            cache:"no-cache",
            credentials:"same-origin",
    ...
    return(<div className="mt-5">
        <Container>
            <Row className="justify-content-center">
                <Button variant="primary" onClick={handleShowCreate}>Create Optics</Button>

                <Modal show={showCreate} onHide={handleCloseCreate}>
                    <Modal.Header closeButton>
                        <Modal.Title>Create Optics</Modal.Title>
                    </Modal.Header>

                    <Form onSubmit={handleCreateBody}>
                        <Modal.Body>
                            <Form.Group controlId="formBasicPassword">
                                <Form.Label>Optics Name</Form.Label>
                                <Form.Control type="text" placeholder="Optics" value={newBodyName} onChange={handleChangeNewBodyName}/>
                            </Form.Group>
                        </Modal.Body>
                        <Modal.Footer>
            ...
            <Modal show={showEdit} onHide={handleCloseEdit}>
                <Modal.Header closeButton>
                    <Modal.Title>Car Optics</Modal.Title>
                </Modal.Header>
                ...
                <Form onSubmit={handleEditBody}>
                    <Modal.Body>
                        <Form.Group controlId="formBasicPassword">
                            <Form.Label>Optics Name</Form.Label>
                            <Form.Control type="text" placeholder="Optics" value={editBodyName} onChange={handleChangeEditBodyName}/>
                        </Form.Group>
                    </Modal.Body>
                    <Modal.Footer>
            ...
            <Modal show={showDelete} onHide={handleCloseDelete}>
                <Modal.Header closeButton>
                    <Modal.Title>Delete Optics</Modal.Title>
                </Modal.Header>
