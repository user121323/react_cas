...
function Salon(){
    ...
    async function loadData(){
        const responseRegion = await fetch("http://localhost:8081/api/carSalon/getAllCarSalon",{
            method:"GET",
            headers:{
    ...
    async function createBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carSalon/addCarSalon",{
            method:"POST",
            mode:"cors",
            cache:"no-cache",
    ...
    async function editBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carSalon/saveCarSalon",{
            method:"PUT",
            mode:"cors",
            cache:"no-cache",
            credentials:"same-origin",
    ...
    async function deleteBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carSalon/deleteCarSalon",{
            method:"DELETE",
            mode:"cors",
            cache:"no-cache",
            credentials:"same-origin",
    ...
    return(<div className="mt-5">
        <Container>
            <Row className="justify-content-center">
                <Button variant="primary" onClick={handleShowCreate}>Create Salon</Button>

                <Modal show={showCreate} onHide={handleCloseCreate}>
                    <Modal.Header closeButton>
                        <Modal.Title>Create Salon</Modal.Title>
                    </Modal.Header>
                    ...
                    <Form onSubmit={handleCreateBody}>
                        <Modal.Body>
                            <Form.Group controlId="formBasicPassword">
                                ...
                                <Form.Control type="text" placeholder="Salon" value={newBodyName} onChange={handleChangeNewBodyName}/>
                            </Form.Group>
                        </Modal.Body>
                        <Modal.Footer>
            ...
            <Modal show={showEdit} onHide={handleCloseEdit}>
                <Modal.Header closeButton>
                    <Modal.Title>Car Salon</Modal.Title>
                </Modal.Header>
                ...
                <Form onSubmit={handleEditBody}>
                    <Modal.Body>
                        <Form.Group controlId="formBasicPassword">
                            <Form.Label>Salon Name</Form.Label>
                            <Form.Control type="text" placeholder="Salon" value={editBodyName} onChange={handleChangeEditBodyName}/>
                        </Form.Group>
                    </Modal.Body>
                    <Modal.Footer>
