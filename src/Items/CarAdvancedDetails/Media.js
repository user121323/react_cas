...
function Media(){
    ...
    async function loadData(){
        const responseRegion = await fetch("http://localhost:8081/api/carMedia/getAllCarMedia",{
            method:"GET",
            headers:{
    ...
    async function createBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carMedia/addCarMedia",{
            method:"POST",
            mode:"cors",
            cache:"no-cache",
    ...
    async function editBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carMedia/saveCarMedia",{
            method:"PUT",
            mode:"cors",
            cache:"no-cache",
            credentials:"same-origin",
    ...
    async function deleteBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carMedia/deleteCarMedia",{
            method:"DELETE",
            mode:"cors",
            cache:"no-cache",
            credentials:"same-origin",
    ...
    return(<div className="mt-5">
        <Container>
            <Row className="justify-content-center">
                <Button variant="primary" onClick={handleShowCreate}>Create Media</Button>

                <Modal show={showCreate} onHide={handleCloseCreate}>
                    <Modal.Header closeButton>
                        <Modal.Title>Create Media</Modal.Title>
                    </Modal.Header>
                    ...
                    <Form onSubmit={handleCreateBody}>
                        <Modal.Body>
                            <Form.Group controlId="formBasicPassword">
                                ...
                                <Form.Control type="text" placeholder="Media" value={newBodyName} onChange={handleChangeNewBodyName}/>
                            </Form.Group>
                        </Modal.Body>
                        <Modal.Footer>
            ...
            <Modal show={showEdit} onHide={handleCloseEdit}>
                <Modal.Header closeButton>
                    <Modal.Title>Car Media</Modal.Title>
                </Modal.Header>
                ...
                <Form onSubmit={handleEditBody}>
                    <Modal.Body>
                        <Form.Group controlId="formBasicPassword">
                            <Form.Label>Media Name</Form.Label>
                            <Form.Control type="text" placeholder="Media" value={editBodyName} onChange={handleChangeEditBodyName}/>
                        </Form.Group>
                    </Modal.Body>
                    <Modal.Footer>
