...
function Outsides(){
    ...
    async function loadData(){
        const responseRegion = await fetch("http://localhost:8081/api/carOutsides/getAllCarOutsides",{
            method:"GET",
            headers:{
    ...
    async function createBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carOutsides/addCarOutsides",{
            method:"POST",
            mode:"cors",
            cache:"no-cache",
    ...
    async function editBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carOutsides/saveCarOutsides",{
            method:"PUT",
            mode:"cors",
            cache:"no-cache",
            credentials:"same-origin",
    ...
    async function deleteBody(data){
        console.log(data)
        const response = await fetch("http://localhost:8081/api/carOutsides/deleteCarOutsides",{
            method:"DELETE",
            mode:"cors",
            cache:"no-cache",
            credentials:"same-origin",
    ...
    return(<div className="mt-5">
        <Container>
            <Row className="justify-content-center">
                <Button variant="primary" onClick={handleShowCreate}>Create Outsides</Button>

                <Modal show={showCreate} onHide={handleCloseCreate}>
                    <Modal.Header closeButton>
                        <Modal.Title>Create Outsides</Modal.Title>
                    </Modal.Header>

                    <Form onSubmit={handleCreateBody}>
                        <Modal.Body>
                            <Form.Group controlId="formBasicPassword">
                                <Form.Label>Outsides Name</Form.Label>
                                <Form.Control type="text" placeholder="Outsides" value={newBodyName} onChange={handleChangeNewBodyName}/>
                            </Form.Group>
                        </Modal.Body>
                        <Modal.Footer>
            ...
            <Modal show={showEdit} onHide={handleCloseEdit}>
                <Modal.Header closeButton>
                    <Modal.Title>Car Outsides</Modal.Title>
                </Modal.Header>
                ...
                <Form onSubmit={handleEditBody}>
                    <Modal.Body>
                        <Form.Group controlId="formBasicPassword">
                            <Form.Label>Outsides Name</Form.Label>
                            <Form.Control type="text" placeholder="Outsides" value={editBodyName} onChange={handleChangeEditBodyName}/>
                        </Form.Group>
                    </Modal.Body>
                    <Modal.Footer>
            ...
            <Modal show={showDelete} onHide={handleCloseDelete}>
                <Modal.Header closeButton>
                    <Modal.Title>Delete Outsides</Modal.Title>
                </Modal.Header>
